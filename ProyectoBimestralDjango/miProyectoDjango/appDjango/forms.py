...
class MayoristaRegistroForm(forms.Form):
    ...
    categoria = forms.ChoiceField(choices=Mayorista.CATEGORIA_CHOICES)
    plan = forms.ChoiceField(choices=Mayorista.PLAN_CHOICES)
...
class TiendaRegistroForm(forms.Form):
    ...
    pin = forms.CharField(label="PIN de seguridad (4 dígitos)", widget=forms.PasswordInput, max_length=4)
    pin_confirmacion = forms.CharField(label="Confirma el PIN", widget=forms.PasswordInput, max_length=4)
    direccion_texto = forms.CharField(label="Dirección referencial", required=False, widget=forms.Textarea(attrs={"rows": 2}))
    ...
    def clean_telefono(self):
        ...
        if User.objects.filter(username=telefono).exists() or Tienda.objects.filter(telefono=telefono).exists():
            raise forms.ValidationError("Ya existe una tienda registrada con ese teléfono.")
        return telefono
    ...
    def clean(self):
        cleaned = super().clean()
        if cleaned.get("pin") and cleaned.get("pin_confirmacion") and cleaned["pin"] != cleaned["pin_confirmacion"]:
            self.add_error("pin_confirmacion", "Los PIN no coinciden.")
        return cleaned

    def guardar(self):
        telefono = self.cleaned_data["telefono"]
        user = User.objects.create_user(username=telefono, password=self.cleaned_data["pin"])
...
class VendedorCrearForm(forms.Form):
    ...
    email = forms.EmailField(label="Correo electrónico")
    tipo_perfil = forms.ChoiceField(choices=Vendedor.TIPO_PERFIL_CHOICES)
    producto_asignado = forms.ModelChoiceField(queryset=Producto.objects.none(), required=False, label="Producto asignado (perfil especializado)")

    def __init__(self, *args, mayorista=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.mayorista = mayorista
        if mayorista is not None:
            self.fields["producto_asignado"].queryset = Producto.objects.filter(mayorista=mayorista)
...
class PedidoEntregaForm(forms.Form):
    nota_voz = forms.CharField(
        label="Nota para la entrega (puedes dictarla con el micrófono)",
        required=False,
    ...
    tipo_pago = forms.ChoiceField(choices=[("efectivo", "Efectivo"), ("digital", "Digital")], widget=forms.RadioSelect)
